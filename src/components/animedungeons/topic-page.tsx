...
import type { TopicPage as TopicPageData } from '@/data/animedungeons/types';
...
export function TopicPage({ topic }: { topic: TopicPageData }) {
  ...
  return (
    <div className="space-y-7 py-5 text-slate-100">
      ...
      <header className="rounded-2xl border border-[#7C5CFF]/25 bg-[radial-gradient(circle_at_top_right,rgba(124,92,255,.24),transparent_44%),#101326] p-6 md:p-8">
        <Badge className="bg-[#46D7FF] text-[#071018]">{topic.eyebrow}</Badge>
      ...
      {topic.sections.map((section) => (
        <section
          key={section.heading}
          className="rounded-xl border border-white/10 bg-[#101326] p-6"
        >
              ...
              {section.links.map((link) =>
                link.href.startsWith('http') ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-2 text-sm font-bold text-[#46D7FF] hover:text-[#9BEAFF]"
                  >
                    {link.label}
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {topic.relatedRoutes.map((route) => (
            <LocaleLink
              key={route}
              href={route}
              className="h-auto min-w-0 whitespace-normal break-words rounded-lg border border-white/10 px-3 py-2 text-left text-sm text-slate-300 hover:border-[#46D7FF]/40 hover:text-[#9BEAFF]"
            >
              {getRouteLabel(route)}
            </LocaleLink>
          ))}
